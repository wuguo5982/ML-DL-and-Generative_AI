...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        pickup = np.random.choice(len(XX), replace = True, size = n).tolist()
        new_dataset = [XX[m] for m in pickup]
        samples = [m[:-1] for m in new_dataset]
        labels = [m[-1] for m in new_dataset]
    ...
    def fitting(self):
        ...
        self.trees = []
        for m in range(len(self.bootstraps_datasets)):
            X_data = self.bootstraps_datasets[m]
            y_label = self.bootstraps_labels[m]
            model = self.decision_trees[m]
            tree = model.learn(X_data, y_label)
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            model = self.decision_trees[0]
            classified_rec = model.classify(record)
            predition = [classified_rec]
